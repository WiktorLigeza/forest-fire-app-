...
# state: 1-alive, 2-burning, 3-burnt
class cell:
    def __init__(self, type, state, iterator, RGB):
        self.type = type
        self.state = state
...
def get_forest(data):
    ...
    for i in range(shape[0]):
        for j in range(shape[1]):
            if data[i][j] > 0:
                arr[i][j] = cell(2, 1, 0, [2, 69, 37])  # forest
            else:
                arr[i][j] = cell(1, 0, 0, [7, 236, 240])  # water
    return arr
    ...
    for i in range(1, shape[0] - 1):
        for j in range(1, shape[1] - 1):
            ...
            if cell.state == 3:
                cell.iterator += 1
                if cell.iterator > 5:
                    cell.state = np.random.choice([1, 3], 1, p=[1 / 2, 1 / 2])  # 50% probability of reviving
                    if cell.state == 1:
                        cell.iterator = 0
                        cell.RGB = [0, 255, 132]
            ...
            # if its normal
            if cell.state == 1:
                # get sum and if sum > 0 ignite with weightet probability of success
                s = chance_of_ignition(data, i, j)
                if s > 0 and s < 5:
                    s = np.random.choice([1, 2], 1, p=[(5 - s) / 5, s / 5])
                    cell.state = s
                elif s > 5:
                    cell.state = 2
    return data
...
def chance_of_ignition(data, i, j):
    s = (data[i - 1][j - 1].state) + (data[i][j - 1].state) + (data[i + 1][j - 1].state) + \
        (data[i - 1][j].state) + (data[i + 1][j].state) + \
        (data[i - 1][j + 1].state) + (data[i][j + 1].state) + (data[i + 1][j + 1].state)
    s -= 8
    return int(s)